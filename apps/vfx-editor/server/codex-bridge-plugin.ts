  ...
  wss.on("connection", (ws) => {
    ...
    ws.on("message", (data) => {
      try {
        ...
        void handleClientMessage(ws, msg, session, (nextSession) => {
          session = nextSession;
        });
      } catch {
...
async function startCodexSession(
  ...
  await sendCodexRequest(session, "initialize", {
    clientInfo: { name: "vfx-editor", title: "VFX Editor", version: "0.1.0" },
...
function handleCodexMessage(
  ...
  switch (msg.method) {
    ...
    case "turn/completed": {
      sendToClient(session.ws, { type: "turn_complete", text: session.agentText });
      cleanupSession(session);
      break;
    }
